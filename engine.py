...
def _build_dashboard(sd, WK_ID, MN1, MN2, MN3, WK1, WK2, WK3, PROJECTS, oa, ot, os_, do_, dc_,
    ...
    dash_html='''<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">'''
    dash_html+=f'<title>Penang Production Dashboard WK{WK_ID}</title>'
    dash_html+='''<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js"></script>'''
    dash_html+=f'<style>body{{font-family:"Segoe UI",sans-serif;margin:20px;background:#f5f5f5}}.hd{{display:flex;align-items:center;justify-content:space-between;margin-bottom:15px;border-bottom:2px solid #1a237e;padding-bottom:10px}}.hl img{{height:48px}}.hb{{font-size:28px;font-weight:700;color:#1a237e;display:inline-block}}.hdt{{font-size:11px;color:#999;margin-top:2px;text-align:right}}h2{{color:#1a237e;font-size:16px;margin:20px 0 10px;border-bottom:2px solid #1a237e;padding-bottom:5px}}.chart-row{{display:flex;flex-wrap:wrap;gap:15px;margin-bottom:15px}}.chart-box{{flex:1 1 450px;background:#fff;padding:15px;border-radius:6px;box-shadow:0 1px 3px rgba(0,0,0,.1)}}.chart-box canvas{{width:100%!important;max-height:300px}}table{{border-collapse:collapse;margin-bottom:15px;box-shadow:0 1px 3px rgba(0,0,0,.1)}}td,th{{border:1px solid #ccc;padding:3px 6px;text-align:center;font-size:11px}}.pj{{text-align:left;font-weight:600;min-width:120px}}.tt{{text-align:center;font-weight:700;color:#fff;background:#1a237e;padding:4px 10px}}.mh{{font-weight:700;background:#e8eaf6;color:#1a237e}}.d{{background:#fff}}.ttl td{{background:#e8eaf6;font-weight:700;color:#1a237e}}.btn{{background:#1a237e;color:#fff;padding:6px 16px;border:0;border-radius:4px;cursor:pointer;font-size:12px;text-decoration:none;display:inline-block;margin:2px}}</style></head><body>'''
    dash_html+=f'<div class="hd"><div><div><h1 style="margin:0;font-size:20px;color:#1a237e">Penang Production Dashboard</h1><div style="font-size:12px;color:#666">NAI Penang - 3-Month Tracking</div></div></div><div><a href="/sum" class="btn">📊 Sum Table</a><div class="hb">WK{WK_ID}</div><div class="hdt">Updated daily</div></div></div>'
    dash_html+=f'<div class="chart-row"><div class="chart-box"><canvas id=crChart></canvas></div><div class="chart-box"><canvas id=naiChart></canvas></div></div>'
    dash_html+=f'<div class="chart-row"><div class="chart-box"><canvas id=otdrChart></canvas></div><div class="chart-box"><canvas id=realPie></canvas></div></div>'
    dash_html+=f'<div class="chart-row"><div class="chart-box"><canvas id=summaryChart></canvas></div><div class="chart-box"></div></div>'
    dash_html+=f'<h2>Project Data</h2><div id=projectTable></div>'
    dash_html+=f'<script>var D=JSON.parse(\'{sd_json.replace("\\","\\\\")}\');</script>'
    dash_html+=f'<script>var pj=D.pj,N=pj.length,WK1=D.WK1,WK2=D.WK2,WK3=D.WK3,TW=WK1+WK2+WK3;var wklbl={json.dumps(wklbl)};var crData=[];pj.forEach(function(p,pi){{var v=[];var o=1;for(var w=0;w<WK1;w++)v.push(D.cr[pi][o+w]);o=7;for(var w=0;w<WK2;w++)v.push(D.cr[pi][o+w]);o=14;for(var w=0;w<WK3;w++)v.push(D.cr[pi][o+w]);crData.push({{label:p,data:v,borderWidth:1,pointRadius:3}});}});new Chart(document.getElementById("crChart"),{{type:"bar",data:{{labels:wklbl,datasets:crData}},options:{{plugins:{{legend:{{labels:{{font:{{size:9}}}}}},title:{{display:true,text:"Customer Request WK"+D.wk}}}},scales:{{x:{{stacked:true}},y:{{stacked:true,beginAtZero:true}}}}}}}});var naiLabels=[],naiData=[];pj.forEach(function(p,pi){{var t=0;for(var w=0;w<TW;w++)t+=D.nai[pi][1+w]||0;t+=D.nai[pi][0]||0;if(t>0){{naiLabels.push(p);naiData.push(t);}}}});new Chart(document.getElementById("naiChart"),{{type:"doughnut",data:{{labels:naiLabels,datasets:[{{data:naiData,backgroundColor:["#1565c0","#ffc107","#4caf50","#ff5722","#9c27b0","#00bcd4","#e91e63","#607d8b"]}}]}},options:{{plugins:{{title:{{display:true,text:"NAI Production by Project"}}}}}}}});var otdrLabels=[],otdrData=[];pj.forEach(function(p,pi){{var v=D.otdr[pi][0]||0;if(v>0){{otdrLabels.push(p);otdrData.push(v);}}}});new Chart(document.getElementById("otdrChart"),{{type:"pie",data:{{labels:otdrLabels,datasets:[{{data:otdrData}}]}},options:{{plugins:{{title:{{display:true,text:"OTDR Pass Due"}}}}}}}});var rLabels=[],rData=[];for(var w=0;w<TW;w++){{var v=D.real_total[w];if(v&&v!==""&&v>0){{rLabels.push(wklbl[w]);rData.push(v);}}}};new Chart(document.getElementById("realPie"),{{type:"bar",data:{{labels:rLabels,datasets:[{{label:"Real Production",data:rData,backgroundColor:"#1565c0"}}]}},options:{{plugins:{{title:{{display:true,text:"Real Production by Week"}}}},scales:{{y:{{beginAtZero:true}}}}}}}});new Chart(document.getElementById("summaryChart"),{{type:"line",data:{{labels:wklbl,datasets:[{{label:"Real Total",data:D.real_total.slice(0,TW),borderColor:"#1565c0",fill:false}},{{label:"OTDR Total",data:D.otdr_tot.slice(0,TW),borderColor:"#ff5722",fill:false}}]}},options:{{plugins:{{title:{{display:true,text:"Real vs OTDR Comparison"}}}},scales:{{y:{{beginAtZero:true}}}}}}}});var ht=\'<table><tr><th class=tt colspan=6>Project Summary</th></tr><tr><th class=mh>Project</th><th class=mh style=text-align:right>CR Total</th><th class=mh style=text-align:right>Pass Due</th><th class=mh style=text-align:right>NAI Shipped</th><th class=mh style=text-align:right>NAI Total</th><th class=mh style=text-align:right>OTDR PD</th></tr>\';pj.forEach(function(p,pi){{var crT=0;var crPd=D.cr[pi][0]||0;for(var j=1;j<D.cr[pi].length;j++)crT+=D.cr[pi][j]||0;var nSh=D.nai[pi][0]||0;var nTot=nSh;for(var j=1;j<D.nai[pi].length;j++)nTot+=D.nai[pi][j]||0;var oPd=D.otdr[pi][0]||0;ht+=\'<tr><td class=pj>\'+p+\'</td><td style=text-align:right>$\'+Number(crT+crPd).toLocaleString()+\'</td><td style=text-align:right>$\'+crPd.toLocaleString()+\'</td><td style=text-align:right>$\'+nSh.toLocaleString()+\'</td><td style=text-align:right>$\'+nTot.toLocaleString()+\'</td><td style=text-align:right>$\'+oPd.toLocaleString()+\'</td></tr>\';}});ht+=\'</table>\';document.getElementById("projectTable").innerHTML=ht;</script>'''
    dash_html+='</body></html>'
    return dash_html
